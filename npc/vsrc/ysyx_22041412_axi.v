...
module ysyx_22041412_axi # (
    ...
    input                               r_valid_i,          //读请求
    input                               w_valid_i,          //写请求
	  output                              r_ready_o,          //读数据结束
    output                              w_ready_o,          //写数据结束
    output [AXI_DATA_WIDTH-1:0]         data_read_o,        //数据输出
    ...
    output reg                          axi_aw_valid_o,  // 主设备给出的地址和相关的控制信号有效
    ...
    output reg                          axi_w_valid_o, // 主设备给出的数据和字节选通信号有效
    ...
    // 写响应通道
    output reg                          axi_b_ready_o, // 主设备已准备好接收写响应信号
    ...
    output reg                          axi_ar_valid_o,  // 主设备给出的地址和相关信息有效
    ...
    // 读数据通道
    output reg                          axi_r_ready_o, // 主设备已经准备好接收读取的数据和响应信息
    ...
    always@(posedge clk)begin
      ...
      end else if(~w_valid_i) axi_aw_valid   <= 1'b0;
    end
    reg [7:0]wd_count;

    /* =============================写数据通道=========================== */
    assign w_ready_o = axi_w_ready_i;
    assign w_last_o  = w_valid_i & (wd_count==w_len_i) ? 1'b1:1'b0;
    reg    w_last_ok;
    always @(posedge clk) begin
      if (rst) begin
        //w_ready_o <= 0;
    ...
    /* =============================读数据通道=========================== */
    assign r_ready_o   = axi_r_valid_i;
    assign data_read_o = axi_r_data_i;
    assign r_last_o    = axi_r_last_i;
    always @(posedge clk) begin
      if (rst) begin
        axi_r_ready_o <= 1'b0;
