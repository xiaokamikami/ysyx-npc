...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", SRAIW     , II, if(BITS(src2,5,5)==0){RR(dest) = SEXT((BITS((int32_t)src1,31,0))>> src2,32);});
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", SRLIW     , II, if(BITS(src2,5,5)==0){RR(dest) = SEXT(BITS(src1,31,0)>> src2 ,32);});
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", SLLIW     , II, if(BITS(src2,5,5)==0){RR(dest) = SEXT(BITS(src1,31,0)<< src2 ,32);});
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", SRAW      , R , RR(dest) = SEXT(BITS((((int32_t)(BITS(src1,31,0))) >> (BITS(src2,4,0))),31,0),32));
  //Load
