...
#define	GET_BIT(x, bit)	BITS(x,bit,bit)	/* Get one bit */
...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT_START();
  //Load
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", LB        , I , RR(dest) = SEXT(BITS(Mr(src1 + src2, 1),7,0),8));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", LBU       , I , RR(dest) = BITS(Mr(src1 + src2, 1),7,0));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", LH        , I , RR(dest) = SEXT(BITS(Mr(src1 + src2, 2),15,0),16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", LHU       , I , RR(dest) = BITS(Mr(src1 + src2, 2),15,0));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", LW        , I , RR(dest) = SEXT(BITS(Mr(src1 + src2, 4),31,0),32));
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", LWU       , I , RR(dest) = BITS(Mr(src1 + src2, 4),31,0));
  INSTPAT("??????? ????? ????? 011 ????? 00000 11", LD        , I , RR(dest) = Mr(src1 + src2,8);
                                                                    //printf("[LD]pc:%lx,%lx read %lx\n" ,Pc,src1 + src2,RR(dest));
                                                                    );
           //0000001 00000 01110 011 01001 00000 11
  //Store     
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", SLLW      , R , RR(dest) = SEXT((BITS(src1,31,0) << BITS(src2,4,0)),32));
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", SRLW      , R , RR(dest) = SEXT((BITS(src1,31,0) >> (BITS(src2,4,0))),32));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", SRAW      , R , RR(dest) = SEXT(((int32_t)(BITS(src1,31,0))>> BITS(src2,4,0)),32));
  
  //移位
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", SLL       , R , RR(dest) = (src1 << BITS(src2,5,0)));
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", SRLIW     , II, if(GET_BIT(src2,5)==0){RR(dest) = SEXT((uint32_t)BITS(src1,31,0)>> src2 ,32);});
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", SRAIW     , II, if(GET_BIT(src2,5)==0){RR(dest) = SEXT(((int32_t)BITS(src1,31,0))>> src2,32);});
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", SLLIW     , II, if(GET_BIT(src2,5)==0){RR(dest) = SEXT((uint32_t)BITS(src1 << src2 ,32,0),32);});
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", MULW      , R , RR(dest) = SEXT(BITS((int32_t)src2 * (int32_t)src1,31,0),32));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", REMW      , R , RR(dest) = SEXT((int32_t)BITS(src1,31,0)%((int32_t)BITS(src2,31,0)),32));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", DIVW      , R , RR(dest) = SEXT((((int32_t)BITS(src1,31,0)) / (int32_t)BITS(src2,31,0)),32));
  ...
  //CSR
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ECALL     , I , Nextpc=isa_raise_intr(0x0b,Pc););
  INSTPAT("0011000 00010 00000 000 00000 11100 11", MRET      , I , Nextpc=csrR_id(mepc));//printf("break a0:%lx\n",RR(10));
  
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", CSRRW     , CSR , t=csrR(src2);csrW(src2,src1);RR(dest)=t);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", CSRRS     , CSR , t=csrR(src2);csrW(src2,t|src1);RR(dest)=t);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", CSRRC     , CSR , t=csrR(src2);csrW(src2,t&(~src1));RR(dest)=t);
  INSTPAT("??????? ????? ????? 101 ????? 11100 11", CSRRWI    , CSR , t=csrR(src2);csrW(src2,(BITS(t,63,5)<<5 | BITS(zimm,4,0)));RR(dest)=t);
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", CSRRSI    , CSR , t=csrR(src2);csrW(src2,(BITS(t,63,5)<<5 | BITS((t|zimm),4,0)));RR(dest)=t); 
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", CSRRCI    , CSR , t=csrR(src2);csrW(src2,(BITS(t,63,5)<<5 | BITS((t&(~zimm)),4,0)));RR(dest)=t);

  //end                    
  INSTPAT("0000000 00001 00000 000 00000 11100 11", EBREAK    , N , NEMUTRAP(s->pc, RR(10))); // R(10) is $a0

  INSTPAT("0000000 00000 00000 001 00000 00011 11", FENCE_I   , N , ); // R(10) is $a0
  //fd 
  //INSTPAT("??????? ????? ????? ??? ????? 10100 11", fsgnd     , N , Log("pc 0x%lx no should run.fsgnd ",Pc););//INV(s->pc)
  //INSTPAT("??????? ????? ????? ??? ????? 01001 11", fsd       , N , Log("pc 0x%lx no should run.fsd ",Pc););//INV(s->pc)
  //INSTPAT("??????? ????? ????? ??? ????? 00001 11", fsd       , N , Log("pc 0x%lx no should run.fld ",Pc););//INV(s->pc)

  //error
  INSTPAT("??????? ????? ????? ??? ????? ????? ??", inv       , N , Log("error in %x ",INSTPAT_INST(s));INV(s->pc));
