...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  //INSTPAT("??????? ????? ????? 110 ????? 11100 11", CSRRCI    , CSR , t=csrR[src2];csrW(src2,src1)(t|zimm);RR(dest)=t); 
  //INSTPAT("??????? ????? ????? 101 ????? 11100 11", CSRRWI    , CSR , t=csrR[src2];csr_regs[src2]=zimm);
  //INSTPAT("??????? ????? ????? 111 ????? 11100 11", CSRRI     , CSR , t=csrR[src2];csr_regs[src2]=(t&(~zimm));RR(dest)=t);
